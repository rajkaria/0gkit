...
4. **Settle** — winners are paid out proportionally. The signed resolution
   receipt is stored in 0G Storage for auditability (immutable
...
- **lib** — `lib/market.ts` (portable `Market`, `Bet`, `Resolution` types,
  `openMarket` / `placeBet` / `resolveMarket` / `settleMarket` helpers).
...
produces an **operator-signed receipt** (EIP-191 signed envelope). There is
no TEE quote verification. The badge is "✓ signature verified". The
`Attestor` interface is injected so a real TEE-quote verifier can slot in
later without changing market code.
