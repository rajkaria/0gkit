...
model, timestamp, extra }`).
3. Signs the receipt with the operator key via
   `@foundryprotocol/0gkit-attestation` (`signEnvelope` / `digestJson`).
   The badge reads **✓ signature verified** — this is an operator-signed
   receipt, not a hardware TEE quote.
...
- **adapters** — one per base (`app/api/oracle/route.ts` for Next.js,
  `src/routes/oracle.ts` for Express, `src/tools/oracle.ts` for MCP).
- **contracts** — `Anchor.sol` + deploy script (opt-in on-chain path only).

## Honesty note

The attestation layer uses **EIP-191 signed envelopes** over the inference
result schema. There is no TEE quote verification in the current stack. The
badge is always "✓ signature verified" — never "TEE attested". The
`Attestor` interface is injected so a real TEE-quote verifier can slot in
