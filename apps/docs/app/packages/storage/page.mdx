...
interface StorageConfig {
  ...
  privateKey?: string; // required for upload (funds the tx)
...
const storage = new Storage({
  network: "galileo",
  privateKey: process.env.ZEROG_PRIVATE_KEY, // funded testnet key
});
...
| Symptom                                                            | Cause                                       | Fix                                                                |
| ------------------------------------------------------------------ | ------------------------------------------- | ------------------------------------------------------------------ |
| `ConfigError: Storage.upload requires a privateKey`                | `upload` with no `privateKey` in config.    | Pass `{ privateKey }` (a funded key) to the `Storage` constructor. |
...
[core](/packages/core) · CLI: [`0g storage`](/cli#0g-storage) · MCP:
`og_storage_*` ([MCP guide](/mcp)) · React:
[`useUpload` / `useDownload`](/react). Template:
