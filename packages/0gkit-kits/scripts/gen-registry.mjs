...
const rawOutput = `// AUTO-GENERATED by scripts/gen-registry.mjs — DO NOT EDIT BY HAND
...
// Format through prettier so the committed generated file stays byte-identical
// to what `pnpm format:check` expects — otherwise every prebuild/pretest
// rewrites it in raw JSON.stringify form (quoted keys, exploded arrays) and the
// format gate fails. Fall back to the raw output if prettier is unavailable
// (keeps a cold build from breaking on a missing/incompatible prettier).
let output = rawOutput;
try {
  const prettierConfig = (await resolveConfig(outFile)) ?? {};
  output = await prettierFormat(rawOutput, {
    ...prettierConfig,
    parser: "typescript",
  });
} catch (err) {
  console.warn(`[gen-registry] prettier formatting skipped: ${err.message}`);
}
