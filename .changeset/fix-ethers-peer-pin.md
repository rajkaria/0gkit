...
Align `ethers` peer dependency with the upstream `@0gfoundation/0g-*-ts-sdk` constraints so consumers can run `npm install` (strict peer resolution) without `ERESOLVE` errors.
