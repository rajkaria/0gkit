 ...
 * HONESTY NOTES
 * ─────────────
 * - These routes are INTENTIONALLY read-only (analyse + log). There is NO
 *   execute/trade/swap/send/transfer endpoint.
 * - Testnet-default: OG_NETWORK defaults to "galileo" (never mainnet/aristotle).
 * - The attestation is a SIGNED RECEIPT — the operator key (OG_PRIVATE_KEY) signs
 *   a canonical digest of the decision receipt. Badge: "✓ signature verified" —
 ...
 *   OG_NETWORK         — "galileo" (testnet default — mainnet is out of scope)
...
// Testnet default: OG_NETWORK=galileo — mainnet is intentionally out of scope.
...
// ---------------------------------------------------------------------------
// POST /api/yield (dispatcher: body.action = "analyze" | "log")
// ---------------------------------------------------------------------------
